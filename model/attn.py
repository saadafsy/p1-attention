...
def emit_lut_svh(path):
    """Write the SAME normative table as a synthesizable SystemVerilog
    combinational ROM (case function), for `include by rtl/online_softmax.sv.

    model/exp_lut.hex stays the normative interchange artifact (docs/uarch.md
    3.5, sha256-pinned); this rendering exists so the RTL needs no $readmemh
    initial block and no cwd-dependent memory-file path across verilator/
    yosys/sby (docs/uarch.md 8.2). Generated only; never hand-edited."""
    lut = gen_exp_lut()
    with open(path, "w") as f:
        f.write("// exp_lut.svh: exp() LUT, UQ1.15, 1024 entries, as a "
                "combinational ROM.\n")
        f.write("// GENERATED by 'python3 model/attn.py --emit-lut-svh "
                "rtl/exp_lut.svh'.\n")
        f.write("// DO NOT EDIT BY HAND. Same table as model/exp_lut.hex "
                "(normative,\n")
        f.write("// docs/uarch.md 3.5): lut[j] = floor(2^15 * exp(-j/2^6) "
                "+ 0.5).\n")
        f.write("// Realization rationale and TB entry-for-entry re-check: "
                "docs/uarch.md 8.2.\n")
        f.write("function automatic logic [15:0] exp_lut_rom("
                "input logic [9:0] j);\n")
        f.write("  case (j)\n")
        for j, v in enumerate(lut):
            f.write(f"    10'd{j}: exp_lut_rom = 16'h{v:04x};\n")
        f.write("    default: exp_lut_rom = 16'h0000;  "
                "// unreachable, 10-bit index is fully enumerated\n")
        f.write("  endcase\n")
        f.write("endfunction\n")
    return lut
...
def main():
    ...
    ap.add_argument("--emit-lut-svh", metavar="PATH",
                    help="write the same table as a synthesizable SV ROM "
                         "include (rtl/exp_lut.svh) and exit")
    args = ap.parse_args()
    did = False
    if args.emit_lut:
        ...
        did = True
    if args.emit_lut_svh:
        emit_lut_svh(args.emit_lut_svh)
        print(f"exp LUT SV ROM written: {args.emit_lut_svh} "
              f"({LUT_SIZE} x 16 bit)")
        did = True
    if not did:
        ap.print_help()
